...
@SuppressLint("HandlerLeak") public class Game extends View {
	...
	protected void onDraw(Canvas canvas){
		...
		for (int i = 0; i < singlesquare.length; i++) {
			...
			for (int j = 0; j < singlesquare[0].length; j++) {
				...
				singlesquare[i][j].draw(canvas, getResources(), j, i, (this.getWidth()+3)/singlesquare.length,this.getHeight()/singlesquare[0].length);
	...
	private boolean validate_game1(){
		...
		for (int i = 0; i < singlesquare.length; i++) {
			...
			for (int j = 0; j < singlesquare[0].length; j++) {
				...
				if (!singlesquare[i][j].equals(anterior)||singlesquare[i][j]instanceof Empty) {
		...
		for (int j = singlesquare[0].length; j >=0; j--) {
			int yau=0;
			for (int z = j; z < singlesquare[0].length; z++) {
				
				if (!singlesquare[yau][z].equals(anterior)||singlesquare[yau][z] instanceof Empty) {
